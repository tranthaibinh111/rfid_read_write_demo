...
void setup() {
	...
	Serial.println(F("Ghi thong tin ca nhan cua the:"));
}
//*****************************************************************************************//
...
void loop() {
  ...
  MFRC522::StatusCode status;
	...
	//-------------------------------------------
	Serial.setTimeout(10000L); // Cho doi 20s
	Serial.println(F("Nhap ten ban (ket thuc bang dau #) : "));
  pointSpace = Serial.readBytesUntil('#', (char *) buffer1, MF_BLOCK_SIZE) ;
  for (byte i = pointSpace; i < MF_BLOCK_SIZE; i++) buffer1[i] = ' ';
  for (byte i = 0; i < MF_BLOCK_SIZE; i++) Serial.print(buffer1[i]);
  Serial.println();
	...
	else Serial.println(F("Xac thuc the thanh cong: "));

	status = mfrc522.MIFARE_Write(block, buffer1, len);
	if (status != MFRC522::STATUS_OK) {
		Serial.print(F("Ghi the that bai: "));
		Serial.println(mfrc522.GetStatusCodeName(status));
	}
	else Serial.println(F("Ghi the thanh cong: "));

	//----------------------------------------
	Serial.println(F("\n**Ket thuc ghi thong tin**\n"));
